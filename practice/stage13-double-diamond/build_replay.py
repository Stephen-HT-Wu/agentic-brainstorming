...
用法：
    python build_replay.py <events.jsonl 路徑> <stage13-run-*.json 路徑> <輸出.html 路徑>
"""
...
# 跟 stage13/graph.py 的 QUANTIFIED_BMC_KEYS 保持一致——「收益流」
...
# stage13/static/index.html 的 STAGE13_COST_SNAPSHOT_ACTIONS 保持一致。
...
def main() -> None:
    if len(sys.argv) != 4:
        print("用法：python build_replay.py <events.jsonl> <stage13-run.json> <輸出.html>")
        sys.exit(1)

"""
Stage 12 專屬的完整回放頁產生器（比照 stage10/build_replay.py 的整體
架構複製再改——不 import stage10，各自維護是這個專案一路以來的既有
慣例，理由見 stage10/build_replay.py 開頭的說明）。

跟 stage10 的版本最大的差異：stage12 的 events.jsonl 每一筆事件的
`extra` 欄位本身就已經是完整資料（draft_idea 帶完整 idea、dfv_score
帶完整批評、baseline 帶完整 proposal＋BMC——這些都是這一輪替 stage12
新增 renderExtraGeneric() case 時就順手做好的），不像 stage9 的舊事件
只有摘要片段、需要另外從 run JSON 「補接」完整資料回事件上
（`_attach_details()`）。stage12 完全不需要這一步。

「六個可量化差異」的對比表也整個換過——stage9 那套是為「多輪同儕互評
+ 共創收斂」設計的（互評次數、跨場記憶引用…），stage12 沒有這些機制，
換成跟這個簡化版真正對應的六個維度：真實搜尋依據、BMC 完整度（贏家
idea 自己設計的那一份——BMC 不是全場共用一份範本，每位參與者各自設計
自己的，見 graph.py `draft_one_idea()` 的說明）、單位經濟、動態組隊
涵蓋的專業角度數、idea 多樣性（pairwise 距離）、DFV 結構化評分覆蓋
——最後再加一列「最終評估者誠實對照評分」，這是整條 pipeline 最終極
的證據，不是只有結構性數字。

用法：
    python build_replay.py <events.jsonl 路徑> <stage12-run-*.json 路徑> <輸出.html 路徑>
"""
...
# 跟 stage12/graph.py 的 QUANTIFIED_BMC_KEYS 保持一致——「收益流」
# 「成本結構」兩格是結構化物件（narrative/monthly_estimate_twd/basis），
# 不是純字串。
QUANT_BMC_KEYS = {"收益流", "成本結構"}

# stage12 的 research_and_team 節點底下 system_research()／generate_personas()
# 共用同一個 instrument() invocation，先發的 system_research 事件只是
# 過程中的快照，真正的累計總額在後面的 generate_personas 事件裡；
# interview_turn 沿用 stage9/10/11 既有的排除慣例。跟
# stage12/static/index.html 的 STAGE12_COST_SNAPSHOT_ACTIONS 保持一致。
COST_SNAPSHOT_ACTIONS = {"interview_turn", "system_research"}
...
def compute_comparison(run_data: dict) -> dict:
    ...
    return {
        ...
        "team_formation": {
            ...
            "agent": f"動態生成 {len(personas)} 位不同專業角度的參與者，各自獨立發想",
        },
        "idea_diversity": {
...
<title>__TITLE__ — Agentic Brainstorming（簡化版）會議回放</title>
<style>
...
<header>
  <h1>__TITLE__</h1>
  <p>Agentic Brainstorming（簡化版）— 5 分鐘腦力激盪會議回放</p>
</header>
<div class="wrap">
...
function renderCompareTable() {
  ...
  const rows = [
    ...
    ["動態組隊", c.team_formation.baseline, c.team_formation.agent],
...
def main() -> None:
    if len(sys.argv) != 4:
        print("用法：python build_replay.py <events.jsonl> <stage12-run.json> <輸出.html>")
        sys.exit(1)
    ...
    html = build_replay_html(
        ...
        personas=run_data.get("personas"), users=(run_data.get("interviewees", []) + run_data.get("evaluators", [])),
    )

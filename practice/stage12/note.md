...
另外兩場真實會議（「APP 訂閱功能」題目）測出 `avg_distance`
...
（例如某場所有 persona 都收斂到同一個命名「PLUS」、同一個 LBS
...
## 誠實的自我檢討：這套 pipeline 相對於「開一份表格叫 LLM 生成」的價值在哪裡

使用者拿一份真實的內部腦力激盪結果做對照測試（同一份表格裡混合真人
提案跟少數用 Gemini 生成的虛擬角色提案，議題性質跟 stage12 類似——
探討既有產品能為合作方帶來什麼獨特價值）。這份表格裡的提案（不管是
真人寫的還是 Gemini 生成的）彼此差異度，主觀上看起來**不輸、甚至可能
高於**我們自己好幾場真實跑測的 `idea_diversity`（0.2382、0.2537、
0.2821，見上方「第二輪修正」）。這是這一輪最重要、也最需要誠實面對的
發現，不是誇大 pipeline 的價值，是承認它目前站不住某些原本以為的優勢。

**重新定位多樣性的真正來源**：一開始以為是「真人有不可複製的獨立
背景」，但發現對照組裡連 LLM 生成的角色彼此差異都不小之後，比較站得住
腳的解釋改成這兩點：
1. 那份表格的題目維持在**開放、抽象的層級**，沒有先被收斂成一個具體
   策略（不像 stage12 的 `analyze_and_scope` 會先把問題收斂成一個
   連產品命名都定了的具體方向，personas 才開始發想）。
2. 角色設定**刻意跨越差異很大的專業領域**（例如設計/工程/研究等完全
   不同學科），不是像 stage12 的 `generate_personas()` 那樣讓 LLM 自己
   判斷「互補團隊」該長什麼樣子——實測發現 LLM 自己發明的「互補」角色
   常常還是同一個大領域裡的不同分工（例如都偏內容/行銷相關），差異的
   「距離」天生比刻意跨領域小很多。

這兩點都是比「策略目標 fan-out」「JTBD 重排序」更輕量、更容易驗證的
假設——不用大改架構，只要讓策略目標的敘述維持開放、讓 persona 生成
明確要求跨領域，就可能拿到接近的效果。**尚未實作**，先記錄這個假設。

**更根本的問題——這整套 pipeline 存在的意義是什麼**：如果一個人打開
一份表格、自己叫 LLM 扮演幾個角色就能拿到差不多的多樣性，那麼建 agent
系統、花 API 成本的意義何在？誠實盤點下來，站得住腳的優勢只有三個，
而且沒有一個是「點子比較好」：

1. **真正獨立、互不污染的平行 context**：一個人手動在同一個對話視窗
   叫模型扮演多個角色，角色之間其實共用同一個 context，會互相污染/
   收斂；要做到真正獨立，得手動開多個分開的對話搬運上下文，麻煩到
   多數人不會真的這樣做。程式碼可以無痛做到真正獨立的平行呼叫，這是
   唯一一個「不是操作者細不細心的問題，是工具能力問題」的差異點。
2. **流程紀律寫死在程式碼裡，不依賴操作者當下夠不夠細心**：一個有
   經驗的人會邊做邊發現「這幾個角色想法太像了」並即時調整；但換一個
   沒那麼有經驗的人來做，品質會大幅浮動。pipeline 把「一定要有訪談
   依據」「一定要盲評」「一定要跟 baseline 對照」這些規則寫死，品質
   下限比較穩定，代價是它現在還沒調對（這整個 session 都在調）。
3. **不用人力就能重複、規模化執行**：偶爾做一次腦力激盪，人自己盯著
   Sheet 叫 LLM 可能更划算；但如果要「每個月對 N 個不同題目都跑一次
   同樣嚴謹的檢驗」，維護一套自動化 pipeline 的人力成本會遠低於每次
   都靠人力盯著跑。這個優勢只有在要規模化、常態化時才會顯現，偶一為之
   的單一題目用不到。

## 反思：Agentic AI 真正被證實有價值的場景是什麼

延伸上面的討論，得出一個更通用的判準：**Agentic AI（多步驟、多次
呼叫、甚至多 agent）真正被證實有價值的場景，幾乎都存在「可驗證的
接地」（verifiable ground truth）——每一步的對錯可以被真實世界的東西
檢查，不是只靠模型自己的意見**。有這個接地，多步驟/多次嘗試/自我修正
才會真的收斂到更好的答案；沒有接地的話，多幾個 agent 只是多幾個一樣
沒有根據的猜測疊加起來，容易產生「看起來量化、其實不是」的假象（這正
是這一整輪追查 `idea_diversity` 數字意義時反覆撞到的陷阱）。

腦力激盪／商業點子評分正好是最沒有客觀對錯的任務之一（DFV 分數是
LLM 自己的意見，不是外部真實回饋），這是它作為 agentic 展示案例天生
偏弱的原因，不是這次實作方法用錯。相對地，業界已經有公開、可驗證、
有名有姓的成功案例，都建立在有客觀對錯可查的任務上：

- **Agentic coding**（例如 SWE-bench 公開基準）：包上「寫程式碼→跑
  測試→看真的過不過→修正→再跑」的迴圈後，成績比單次回答顯著提升，
  因為「測試過不過」是客觀事實，不是模型自己說了算。這個 session 本身
  就是活生生的例子——每個真實 bug（LangGraph join-semantics、529
  過載）跟每次修復驗證，都有「跑起來對不對」這個外部答案可查。
- **RAG**（企業知識庫、法律研究工具）：價值是「回答有沒有根據真實
  文件」，判準是引用是否存在、是否寫得一致，可以點進去查證，不是主觀
  評價。
- **Deep Research 類多步驟搜尋 agent**：多次搜尋、交叉比對、整理成
  有引用的報告，價值是「有沒有真的查到、查對了」，一樣可驗證。
- **合規／安全稽核類多 agent 檢核**：表面上很像 DFV 三面向評審（多個
  角色各自檢查一個面向），但差異巨大——安全掃描的判準是「是否符合
  已知的客觀規則」，不是「使用者想不想要」這種純猜測。**同一種「多個
  獨立評審」的架構，套在有客觀答案的任務上有效，套在純主觀判斷的任務
  上就是這次踩到的坑**。

**對這個專案／這次 demo 的誠實啟示**：敘事應該從「這套流程想得比一群
人腦力激盪更好」（目前證據撐不住）調整成「這是一個 agentic 編排技術
的能力展示，價值在可驗證的接地任務（寫程式、查資料、重複性操作）上
最紮實，商業發想是這項技術目前相對偏弱的展示場景」。

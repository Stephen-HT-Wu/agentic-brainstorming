"""
Stage 12：即時控制面板的 web server（比照 stage11 server.py 複製再改）。

不直接 import stage12/graph.py 來跑會議——每個「跑一段」的動作都透過
...
- 跑完的會議提供最終報告 .md，以及完整事件回放頁（`stage12/build_replay.py`
  是 stage12 自己的一份，不是 stage10 那份——node 拓樸/state 形狀完全
  不同，不能共用同一套 compute_comparison()，見 build_replay.py 開頭
  的說明）
"""
...
STAGE12_DIR = Path(__file__).resolve().parent
PRACTICE_DIR = STAGE12_DIR.parent
RUNS_DIR = PRACTICE_DIR / "outputs" / "runs"
WORKER_SCRIPT = STAGE12_DIR / "run_worker.py"
STATIC_DIR = STAGE12_DIR / "static"
...
sys.path.insert(0, str(STAGE12_DIR))
...
def index():
    ...
    if not path.exists():
        return HTMLResponse("<h1>Stage 12</h1><p>static/index.html 還沒建立。</p>")
...
def shared_renderers_js():
    ...
    # 同一份原始碼，不是複製過來的）——stage12/build_replay.py 已經在載入
...
def get_replay(run_id: str):
    ...
    # 逐步重播每一步、看結構化差異對照表（stage12/build_replay.py）。
    run_dir = _run_dir(run_id)
    state = _current_status(run_id)
    if state.get("status") != "done":
    ...
    # 訪談對象＋最終評估者都是「使用者」身分，合併成一份清單給
    # findUser()／renderUserProfile() 查找（id 前綴 u.../e... 不會互相
    # 覆蓋，跟 static/index.html 即時畫面同一套慣例）。
    users = (run_data.get("interviewees") or []) + (run_data.get("evaluators") or [])

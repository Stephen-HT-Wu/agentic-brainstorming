...
- 跑完的會議提供最終報告 .md（stage12 沒有 stage10 風格的完整事件回放頁
  ——compute_comparison() 那套是照 stage9 的資料形狀寫的，stage12 的節點
  拓樸/state 形狀完全不同，硬套會全部對不上；「報告 .md ＋ 已經串流過的
  即時畫面事件記錄」對這個簡化版來說就足夠了，見 stage12/note.md）
"""
...
STAGE10_DIR = PRACTICE_DIR / "stage10"
...
def get_report(run_id: str):
    # stage12 沒有 stage10 風格的完整事件回放頁（見檔案開頭說明），跑完後
    # 「回顧」用的是這份最終報告 .md ＋ 即時畫面已經串流過的事件記錄，兩者
    # 合起來就足夠，不用另外組一個 build_replay 風格的靜態 HTML production。
    run_dir = _run_dir(run_id)
    state = _current_status(run_id)

"""
Stage 12 的執行單位（比照 stage11 run_worker.py 原封不動複製再改）：每次
「開始一場會議」或「回答/跳過一次人類提問」都是獨立的一個 subprocess 呼叫，
不是常駐 process 裡的一條 thread——理由跟 stage11 完全一樣：graph.py 的
OUTPUT_DIR／EVENTS_PATH／CHECKPOINT_DB_PATH 等路徑是模組層級變數，同一個
process 裡如果同時有兩場會議的 thread 都在跑，monkeypatch 會互相覆蓋。

跟 stage11 的差異：這次 graph.py 就在同一個目錄下（stage12 是完整獨立副本，
不像 stage11 沒有自己的 pipeline、要從 stage9 import），不用 `sys.path`
額外插入路徑；也拿掉了 Chroma 隔離邏輯（stage12 整個沒有集體智慧庫）。
"""
...
def _refresh_dynamic_snapshot(state: dict) -> None:
    """stage12 的腦力激盪參與者／訪談對象／最終評估者三組人都是動態生成
    ...
    `analyze_and_scope`／`research_and_team` 是父圖最前面兩個節點，一定
    會在第一個（也是唯一一個）人類介入點 `ask_question` 之前就跑完，
    所以只要這些欄位非空，這裡拿到的就已經是真正動態生成的名單——把它
    寫回 config_snapshot.json，即時畫面重新整理／首次載入時
    loadRunConfig() 才不會拿到跟真實訪談對象/評估者對不上的舊快照。
    訪談對象跟最終評估者的 id 前綴不同（u1.../e1...），直接合併成一份
    `users` 清單給前端的 `findUser()` 查找，不會互相覆蓋。"""
    personas = state.get("personas")
    interviewees = state.get("interviewees") or []
...
def _inspect_final_state(thread_id: str) -> dict:
    ...
    if not snapshot.next:
        ...
        run_json_matches = sorted(sg.OUTPUT_DIR.glob("stage12-run-*.json"))
        if report_matches:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Stage 12 會議 worker：跑一段直到下一個人類介入點或結束")

...
`interviewees` 欄位）；存檔檔名前綴改成 `stage14-run-*.json`。
"""
...
def _refresh_dynamic_snapshot(state: dict) -> None:
    """stage14-signals 的腦力激盪參與者／訪談對象／最終評估者三組人都是動態生成
...
def _inspect_final_state(thread_id: str) -> dict:
    ...
    if not snapshot.next:
        ...
        run_json_matches = sorted(sg.OUTPUT_DIR.glob("stage14-run-*.json"))
        if report_matches:
...
def cmd_start(args: argparse.Namespace) -> None:
    ...
    if args.interview_mode:
        os.environ["BRAINSTORM_INTERVIEW_MODE"] = args.interview_mode
    if args.n_interviewees:
        os.environ["BRAINSTORM_N_INTERVIEWEES"] = str(args.n_interviewees)
    if args.survey_n:
...
def main() -> None:
    ...
    p_start.add_argument("--interview-mode", choices=["diverse", "topic_fit"], default=None)
    p_start.add_argument("--n-interviewees", type=int, default=None)

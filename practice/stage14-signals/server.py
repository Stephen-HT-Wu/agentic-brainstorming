...
不直接 import stage14-signals/graph.py 來跑會議——每個「跑一段」的動作都透過
...
- 跑完的會議提供最終報告 .md，以及完整事件回放頁（`stage14-signals/build_replay.py`
...
class CreateRunRequest(BaseModel):
    ...
    interview_mode: str = "topic_fit"
    n_interviewees: int = 2
    survey_n: int = 8
...
def shared_renderers_js():
    ...
    # 同一份原始碼，不是複製過來的）——stage14-signals/build_replay.py 已經在載入
...
def create_run(body: CreateRunRequest):
    ...
    args = [sys.executable, str(WORKER_SCRIPT), "start",
            ...
            "--interview-mode", body.interview_mode, "--n-interviewees", str(body.n_interviewees),
...
def get_replay(run_id: str):
    ...
    # 逐步重播每一步、看結構化差異對照表（stage14-signals/build_replay.py）。
    run_dir = _run_dir(run_id)
    state = _current_status(run_id)
    if state.get("status") != "done":

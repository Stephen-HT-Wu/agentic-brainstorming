...
stage14-signals 用 Double Diamond 重排節點順序：
...
# 從根本上避開同一個問題。
EVENTS_PATH = OUTPUT_DIR / "stage14_events.jsonl"
CHECKPOINT_DB_PATH = OUTPUT_DIR / "stage14_checkpoints.sqlite"
...
EMBED_DIM = 256
# stage14-signals 把 Discover 階段的訪談從「驗證某個預設方向」改成 JTBD 的
...
N_INTERVIEWEES_PER_JOB = 2    # 每個候選 job 各自訪談的人數（跑時可被
                               # state["n_interviewees_per_job"] 覆蓋，這裡
                               # 只是沒有指定時的預設值，見
                               # desk_research_hypothesize_jobs() 的說明）
N_PERSONAS = 5                 # Develop 階段扣著公司能力衍生職能的參與
                                # 者數（從 stage12 的 3 提高，畢竟沒有
                                # 成本上限，且職能覆蓋度值得多留幾個席位）
N_EVALUATORS = 3

# stage14-signals 新增：訪談對象的生成視角可切換，不再只有一種寫死的
# 生成方式。「topic_fit」貼近候選 job 情境（但彼此仍要有年齡/性別/情境
# 差異，不能像同一人的兩個版本）；「diverse」刻意拉開差異、涵蓋不會被
# 直覺聯想到這個主題的邊緣使用者/懷疑論者/非典型情境——目的是測試這個
# 候選 job 假設在更廣泛母體裡站不站得住腳，不是只找立場一致的人。
DEFAULT_INTERVIEW_MODE = "topic_fit"
INTERVIEW_MODE_PROMPT_FRAGMENTS = {
    "topic_fit": (
        "每位受訪者的背景/情境要能合理對應到這個候選 job 的情境（貼近"
        "可能重度遇到這個情境的使用者樣態），但彼此仍要在年齡/性別/生活"
        "情境上有變化，避免看起來像同一人的兩個版本。"
    ),
    "diverse": (
        "每位受訪者的年齡、性別、職業類型、生活情境要盡量拉開差異，"
        "刻意涵蓋不會被直覺聯想到這個主題的邊緣使用者/懷疑論者/非典型"
        "情境，不要求每個人都明顯適配這個候選 job——目的是測試這個候選"
        "job 假設在更廣泛母體裡是否依然成立，不是只找立場一致的人。"
    ),
}
...
# 挖）；stage14-signals 的 Discover 階段要檢驗的是「這個候選 job 假設是不是
...
class MeetingState(TypedDict):
    ...
    company: str
    round_id: str
    # stage14-signals 新增：訪談對象生成的設定（見
    # INTERVIEW_MODE_PROMPT_FRAGMENTS 的說明），有預設值，不是必填——
    # main() 從環境變數讀、其餘進入點（CLI 單元測試等）不給時直接沿用
    # DEFAULT_INTERVIEW_MODE／N_INTERVIEWEES_PER_JOB。
    interview_mode: Literal["diverse", "topic_fit"]
    n_interviewees_per_job: int
    # ---- Discover（發散）----
    ...
    # run_virtual_survey 產出：虛擬問卷的方向性訊號（見 SURVEY_METHOD_CAVEAT
    # 的說明），是質化訪談證據之外的量化補充，不是取代——插在
    # discover_and_evaluate_jobs 跟 select_job_and_define_problem 之間，
    # 拓樸上仍是嚴格單一前驅鏈，不是新的平行分支
...
def desk_research_hypothesize_jobs(state: MeetingState) -> dict:
    ...
    stage14-signals 新增：訪談對象的生成視角（`interview_mode`）跟人數
    （`n_interviewees_per_job`）改成從 `state` 讀（有預設值），不是寫死
    的模組常數——見 `INTERVIEW_MODE_PROMPT_FRAGMENTS` 的說明。"""
    topic = state["topic"]
    company = state["company"]
    interview_mode = state.get("interview_mode") or DEFAULT_INTERVIEW_MODE
    n_interviewees = state.get("n_interviewees_per_job") or N_INTERVIEWEES_PER_JOB
    mode_fragment = INTERVIEW_MODE_PROMPT_FRAGMENTS.get(
        interview_mode, INTERVIEW_MODE_PROMPT_FRAGMENTS[DEFAULT_INTERVIEW_MODE],
    )
    ...
    try:
        ...
        system = (
            ...
            f"不是公司內部人員），恰好 {n_interviewees} 位。{mode_fragment}"
            "只輸出JSON：{"
        ...
        if len(candidate_jobs) < 2:
            ...
            for n in range(1, N_CANDIDATE_JOBS + 1):
                start = (n - 1) * n_interviewees
                pool = fallback_users[start:start + n_interviewees] or fallback_users[:n_interviewees]
                candidate_jobs.append({
                    "id": f"job{n}",
        ...
        for cj in candidate_jobs:
            if len(cj["interview_pool"]) < 1:
                cj["interview_pool"] = load_users()[:n_interviewees]
...
# ---------------------------------------------------------------------------
# 虛擬問卷（stage14-signals 新增）：質化訪談證據之外的量化補充訊號
# ---------------------------------------------------------------------------
...
# 這段警語必須原封不動跟著 survey_summary 一起傳遞到報告/回放/Define
# 收斂的 prompt 裡——不管哪一層下游用到這份資料，都不能把它靜默丟掉。
SURVEY_METHOD_CAVEAT = (
    "以下是虛擬問卷產生的方向性訊號：由單一 LLM 內部分布模擬出的虛擬受訪者，"
    "不是真實人類母體的獨立抽樣，不具統計顯著性，只能當作質化訪談證據之外的"
    "『參考訊號』，不能單獨用來推翻質化證據，也不能被包裝成有統計意義的真實數據。"
)
...
def fan_out_survey_strata(state: SurveyPanelState) -> List[Send]:
    return [
        Send("survey_one_stratum", {
            "stratum": stratum, "candidate_jobs": state["candidate_jobs"],
            "topic": state["topic"], "n_respondents": state["n_respondents"],
        })
...
def survey_one_stratum(task: SurveyStratumTask) -> dict:
    ...
    對每個候選 job 給出彙整後的統計量（不是逐字稿）——不對每一位虛擬
    受訪者各自 fan-out，避免撞 rate limit、拉長壁鐘時間（stage12/note.md
    已經分析過這個障礙）。"""
    stratum = task["stratum"]
    candidate_jobs = task["candidate_jobs"]
    n_respondents = task["n_respondents"]
    role_token = _event_role.set(f"survey:{stratum['label']}")
    jobs_block = "\n".join(f"- [{cj['id']}] {cj['job_statement']}" for cj in candidate_jobs)
    system = (
        f"你要模擬「{stratum['label']}」這個人口特徵分層裡 {n_respondents} 位"
        "虛擬受訪者，針對下面每個情境陳述，估計這個分層裡的人對這個情境感到"
        "持續困擾的比例、以及困擾強度。這是模擬訊號，不是真實調查，但仍要"
        "誠實反映這個分層的合理差異，不要對每個情境都給同樣的數字。只輸出"
        "JSON：{\"jobs\": [{\"job_id\":\"...\",\"high_distress_pct\":0-100的數字"
        "（這個分層裡對此情境感到持續困擾的模擬比例）,\"avg_intensity\":1-5的"
        "數字（模擬平均困擾強度）,\"sample_quote\":\"<=40字，一則標明是模擬的"
        "代表性引述，或空字串\"}]}"
    )
    user = f"情境陳述清單：\n{jobs_block}"
    try:
    ...
    jobs_by_id = {j.get("job_id"): j for j in (data.get("jobs") or []) if isinstance(j, dict)}
    used_fallback = False
    job_stats = []
    for cj in candidate_jobs:
        j = jobs_by_id.get(cj["id"])
        if j is None:
            used_fallback = True
            job_stats.append({
                "job_id": cj["id"], "job_statement": cj["job_statement"],
                "high_distress_pct": 50.0, "avg_intensity": 3.0,
                "sample_quote": "",
            })
            continue
        try:
            pct = max(0.0, min(100.0, float(j.get("high_distress_pct"))))
        except (TypeError, ValueError):
            pct = 50.0
            used_fallback = True
        try:
            intensity = max(1.0, min(5.0, float(j.get("avg_intensity"))))
        except (TypeError, ValueError):
            intensity = 3.0
            used_fallback = True
        job_stats.append({
            "job_id": cj["id"], "job_statement": cj["job_statement"],
            "high_distress_pct": pct, "avg_intensity": intensity,
            "sample_quote": _safe_str(j.get("sample_quote")),
        })
...
def _aggregate_survey_results(survey_results: List[dict], candidate_jobs: List[dict]) -> dict:
    """依 n_simulated 加權平均每個候選 job 的統計量，回傳的 dict 一律內含
    SURVEY_METHOD_CAVEAT 原文，保證不會在往後傳遞時被靜默丟掉。"""
    by_job: dict = {}
    for res in survey_results:
        n = res["n_simulated"]
        for js in res["job_stats"]:
            acc = by_job.setdefault(js["job_id"], {
                "weighted_pct": 0.0, "weighted_intensity": 0.0, "total_n": 0, "quotes": [],
            })
            acc["weighted_pct"] += js["high_distress_pct"] * n
            acc["weighted_intensity"] += js["avg_intensity"] * n
            acc["total_n"] += n
            if js.get("sample_quote") and len(acc["quotes"]) < 2:
                acc["quotes"].append(f"[{res['stratum_label']}] {js['sample_quote']}")

    job_statement_by_id = {cj["id"]: cj["job_statement"] for cj in candidate_jobs}
    by_job_summary = {}
    for job_id, acc in by_job.items():
        total_n = acc["total_n"] or 1
        by_job_summary[job_id] = {
            "job_statement": job_statement_by_id.get(job_id, ""),
            "high_distress_pct": round(acc["weighted_pct"] / total_n, 1),
            "avg_intensity": round(acc["weighted_intensity"] / total_n, 2),
            "sample_quotes": acc["quotes"],
        }
    return {
        "caveat": SURVEY_METHOD_CAVEAT,
        "total_simulated_n": sum(res["n_simulated"] for res in survey_results),
        "by_job": by_job_summary,
    }


def run_virtual_survey(state: MeetingState) -> dict:
    """插在 discover_and_evaluate_jobs 跟 select_job_and_define_problem
    之間的循序節點（不是平行分支——見 build_parent_graph() 的拓樸安全性
    說明），內部同步呼叫 survey_panel_graph（模式 B），再用模式 A fan-out
    到 survey_one_stratum，遞迴組合跟 discover_and_evaluate_jobs 完全
    一致，不發明新的 join。"""
    n_respondents = state.get("survey_respondents_per_stratum") or DEFAULT_SURVEY_RESPONDENTS_PER_STRATUM
    result = survey_panel_graph.invoke({
        "topic": state["topic"],
        "candidate_jobs": state["candidate_jobs"],
    ...
    summary = _aggregate_survey_results(survey_results, state["candidate_jobs"])
    print(f"  [run_virtual_survey] {len(survey_results)} 個分層，總模擬樣本數 {summary['total_simulated_n']}")
    # 使用者在即時畫面上只看得到各分層各自的 survey_one_stratum 事件，
    # 看不到彙整後、真正餵給 select_job_and_define_problem 的 survey_summary
    # ——這裡補發一則彙整事件，讓即時畫面／回放頁都能直接看到最終數字，
    # 不用自己心算加權平均。
    emit_event(
        "run_virtual_survey",
        f"虛擬問卷彙整完成（{len(survey_results)} 個分層，總模擬樣本數 {summary['total_simulated_n']}）",
        extra={"survey_summary": summary},
    )
    return {"survey_results": survey_results, "survey_summary": summary}
...
def select_job_and_define_problem(state: MeetingState) -> dict:
    ...
    survey_summary = state.get("survey_summary") or {}
    survey_block = ""
    if survey_summary.get("by_job"):
        survey_lines = "\n".join(
            f"- [{job_id}] 模擬高困擾比例 {stats['high_distress_pct']}%，"
            f"模擬平均困擾強度 {stats['avg_intensity']}/5"
            + (f"，模擬引述：{'; '.join(stats['sample_quotes'])}" if stats.get("sample_quotes") else "")
            for job_id, stats in survey_summary["by_job"].items()
        )
        survey_block = (
            f"\n\n量化補充（虛擬問卷，模擬訊號，非真人樣本，僅供參考，"
            f"總模擬樣本數 {survey_summary.get('total_simulated_n', 0)}）：\n{survey_lines}\n"
            f"{survey_summary.get('caveat', SURVEY_METHOD_CAVEAT)}"
        )
    ...
    system = (
        ...
        "（不能是空泛判斷）。如果有提供量化補充（虛擬問卷），可以當作"
        "質化訪談證據之外的參考訊號，但那是模擬數據、不具統計顯著性，"
        "嚴禁在 why_selected 裡把它講成有統計意義的真實調查結果，也不能"
        "單獨用量化補充推翻質化訪談證據——質化證據永遠是主要依據。定義出"
        "這個 job 對應的 target_audience（終端使用者輪廓），寫一份解法無關"
        "的 problem_statement（POV 風格：情境+使用者+未被滿足的需求+為什麼"
        "現狀不滿意），以及一句 How Might We（hmw，銜接後續發想，但本身也"
        "不能點名解法）。只輸出 JSON：{"
        "\"selected_job_id\":\"...\",\"why_selected\":\"<=100字，引用具體證據\","
        "\"target_audience\":\"<=80字\",\"problem_statement\":\"<=150字，"
        "不能出現解法/產品名稱\",\"hmw\":\"<=40字，How Might We 開頭\"}"
    )
    user = f"候選 job 與證據：\n{evidence_block}{survey_block}"
    raw = call_llm(SMART_MODEL, system, user, max_tokens=1500)
    data = extract_json_object(raw)
    if not data:
        data = extract_json_object(repair_json_text(raw))
...
def save_outputs(
    *,
    ...
    path = OUTPUT_DIR / f"stage14-run-{stamp}.json"
    path.write_text(json.dumps(out, ensure_ascii=False, indent=2), encoding="utf-8")
    (OUTPUT_DIR / f"stage14-latest-{topic[:12]}.json").write_text(
...
def build_final_report_markdown(
    *,
    ...
    if survey_summary.get("by_job"):
        ...
        for job_id, stats in survey_summary["by_job"].items():
            L.append(f"- [{job_id}] {stats.get('job_statement', '')}")
            L.append(
                f"  - 模擬高困擾比例：{stats.get('high_distress_pct')}%，"
                f"模擬平均困擾強度：{stats.get('avg_intensity')}/5"
            )
...
def build_parent_graph(checkpointer):
    ...
    g.add_node(
        "run_virtual_survey",
        instrument("run_virtual_survey", run_virtual_survey),
    )
    g.add_node(
        "select_job_and_define_problem",
    ...
    g.add_edge("discover_and_evaluate_jobs", "run_virtual_survey")
    g.add_edge("run_virtual_survey", "select_job_and_define_problem")
...
def main() -> None:
    ...
    interview_mode = os.environ.get("BRAINSTORM_INTERVIEW_MODE", DEFAULT_INTERVIEW_MODE)
    n_interviewees = int(os.environ.get("BRAINSTORM_N_INTERVIEWEES", str(N_INTERVIEWEES_PER_JOB)))
    ...
    final_state = run_meeting(
        ...
        config,
        {
            ...
            "n_interviewees_per_job": n_interviewees,
    ...
    out_path = save_outputs(
        ...
        survey_results=final_state["survey_results"], survey_summary=final_state["survey_summary"],
    ...
    report_md = build_final_report_markdown(
        ...
        candidate_jobs=candidate_jobs, used_fallback_candidate_jobs=final_state["used_fallback_candidate_jobs"],
        job_evidence=final_state["job_evidence"],
        survey_summary=final_state["survey_summary"],
        selected_job_id=final_state["selected_job_id"], selected_job=final_state["selected_job"],
        target_audience=final_state["target_audience"], problem_statement=final_state["problem_statement"],
        hmw=final_state["hmw"],
        five_forces=final_state["five_forces"], trend_analysis=final_state["trend_analysis"],

"""
Stage 13 專屬的完整回放頁產生器（比照 stage12/build_replay.py 的整體
架構複製再改——不 import stage12，各自維護是這個專案一路以來的既有
慣例，理由見 stage10/build_replay.py 開頭的說明）。

跟 stage12 的版本最大的差異：Double Diamond 重排後 state 形狀完全不同
（`strategic_goal` 換成 `candidate_jobs`/`job_evidence`/`selected_job`/
`problem_statement`/`hmw`，訪談對象巢狀在每個候選 job 的
`interview_pool` 裡，不是扁平的 `interviewees`），所以「六個可量化
差異」對比表裡「動態組隊」這一列改成報告從公司能力衍生出的職能覆蓋度
（不重複，見 graph.py `derive_company_domains()` 的說明），另外新增一列
「候選 job 覆蓋率」（假設幾個、選定哪一個、其餘的訪談證據仍然可稽核）。
其餘維度（真實搜尋依據、BMC 完整度、單位經濟、idea 多樣性、DFV 結構化
評分覆蓋、最終評估者誠實對照評分）跟 stage12 同一套資料形狀，直接沿用。

stage13 也不再有 stage12 那種「兩個節點函式折進同一個 instrument()
invocation」的寫法（`research_and_team` 已經拆回 JTBD 重排序後天生
不需要 join 的獨立節點，見 graph.py `build_parent_graph()` 的說明），
所以 `COST_SNAPSHOT_ACTIONS` 只需要排除 `interview_turn`（同一位受訪者
5 輪 switch 訪談共用一個 invocation，每輪事件的 cost_usd 是「這個
invocation 至今累計」，全部加總會重複計入），不需要再排除任何「mid
snapshot」事件。

用法：
    python build_replay.py <events.jsonl 路徑> <stage14-run-*.json 路徑> <輸出.html 路徑>
"""
...
# 跟 stage14-signals/graph.py 的 QUANTIFIED_BMC_KEYS 保持一致——「收益流」
...
# 每則事件的 cost_usd 是「這個 invocation 至今累計」，不是單則增量，跟
# stage12 的 system_research／generate_personas 是同一種「mid-snapshot」
# 模式：真正的累計總額在迴圈結束後的 user_evaluation_summary 事件裡，
# evaluate_final_outputs 整批都是重複計入的中途快照，要整批排除——跟
# stage14-signals/static/index.html 的 STAGE14_COST_SNAPSHOT_ACTIONS 保持一致。
COST_SNAPSHOT_ACTIONS = {"evaluate_final_outputs"}


def sum_display_cost(events: list) -> float:
    """真實跑測撞到的坑：`interview_turn` 沒有這種「有一個獨立動作名稱
    的最終快照事件」可以排除法——同一位受訪者 5 輪 switch 訪談的每一輪
    都叫同一個動作名稱 `interview_turn`，且每輪的 cost_usd 一樣是「這個
    invocation 至今累計」，用 COST_SNAPSHOT_ACTIONS 整批排除會把這位
    受訪者的訪談成本整個從加總消失，而不是排除法通常假設的「只排除
    重複計入的部分」。修法：不是整批排除，而是每位受訪者只留最後一輪
    （round 最大的那一則，累計數字就是這次訪談的真實總額），其餘輪次
    才真的丟棄——這樣才不會把訪談成本整個漏算。"""
    total = 0.0
    last_interview_turn: dict = {}
    for e in events:
        if e.get("action") == "interview_turn":
            extra = e.get("extra") or {}
            # 用 user_name 而非 user_id 分組：`interview_turn` 事件寫入
            # events.jsonl 的當下還沒有 candidate_job_id 標籤（那是
            # `research_one_candidate_job()` 事後才補寫到回傳的逐字稿
            # 物件上，emit_event 早就把 JSON 序列化寫進磁碟了），而
            # `user_id` 是每個候選 job 各自從 u1/u2 起算，跨候選 job 會
            # 撞名——同一場真實跑測就出現過 3 個不同候選 job 都有
            # user_id="u1" 的情況。user_name 是 LLM 為整場會議生成的，
            # 實務上跨候選 job 撞名機率低得多。
            key = extra.get("user_name") or extra.get("user_id")
            round_i = extra.get("round", 0)
            prev = last_interview_turn.get(key)
            if prev is None or round_i >= prev[0]:
                last_interview_turn[key] = (round_i, e.get("cost_usd", 0.0))
            continue
        if e.get("action") in COST_SNAPSHOT_ACTIONS:
            continue
        total += e.get("cost_usd", 0.0)
    total += sum(cost for _, cost in last_interview_turn.values())
    return total
...
def compute_comparison(run_data: dict) -> dict:
    ...
    candidate_jobs = run_data.get("candidate_jobs") or []
    job_evidence = run_data.get("job_evidence") or []
    selected_job_id = run_data.get("selected_job_id")
    supported_count = sum(1 for ev in job_evidence if ev.get("supported"))
    domains = [p.get("domain") for p in personas if p.get("domain")]

    survey_summary = run_data.get("survey_summary") or {}
    survey_by_job = survey_summary.get("by_job") or {}
    selected_survey_stats = survey_by_job.get(selected_job_id)
    ...
    return {
        ...
        "candidate_job_coverage": {
            "baseline": "N/A（沒有 Discover/Define 階段）",
            "agent": (
                f"假設 {len(candidate_jobs)} 個候選 job，{supported_count} 個訪談後判定成立，"
                f"選定 {selected_job_id}（其餘候選 job 的訪談證據仍可稽核，見回放事件）"
            ),
        },
        "idea_diversity": {
        ...
        "virtual_survey": {
            ...
            "agent": (
                ...
                f"選定 job 模擬高困擾比例 {selected_survey_stats['high_distress_pct']}%"
                if selected_survey_stats else "（本次未產生虛擬問卷資料）"
            ),
        },
        "dfv_scoring": {
            "baseline": "無結構化評分",
...
<title>__TITLE__ — Agentic Brainstorming（Double Diamond）會議回放</title>
<style>
...
<header>
  <h1>__TITLE__</h1>
  <p>Agentic Brainstorming（Double Diamond）— Discover→Define→Develop→Deliver 會議回放</p>
</header>
<div class="wrap">
...
function renderCompareTable() {
  ...
  const rows = [
    ...
    ["候選 job 覆蓋率（Discover/Define）", c.candidate_job_coverage.baseline, c.candidate_job_coverage.agent],
...
def main() -> None:
    if len(sys.argv) != 4:
        print("用法：python build_replay.py <events.jsonl> <stage14-run.json> <輸出.html>")
        sys.exit(1)

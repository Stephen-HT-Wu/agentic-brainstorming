"""
Stage 15（stage15-market-fit）：即時控制面板的 web server（比照
stage14-signals server.py 複製再改）。

不直接 import stage15-market-fit/graph.py 來跑會議——每個「跑一段」的動作都透過
...
- 跑完的會議提供最終報告 .md，以及完整事件回放頁（`stage15-market-fit/build_replay.py`
  是 stage15-market-fit 自己的一份，不是更早 stage 的那份——各 stage
...
def _company_slugify(name: str) -> str:
    """跟 _slugify() 同樣只留 ASCII 字元的邏輯，但保底字串不能沿用
    _slugify() 的 "topic"——中文公司名稱（很常見的情況，沒有 ASCII 字元）
    落到保底時，寫出來的檔名會變成誤導人的 topic-company.md，讓人以為
    調查失敗或抓錯東西。改成帶時間戳的 company-<timestamp>，至少看得出
    是「公司研究」的產出，不會跟其他保底字串搞混。"""
    s = re.sub(r"[^A-Za-z0-9]+", "-", name).strip("-").lower()
    return s[:24] or datetime.now().strftime("co-%Y%m%d-%H%M%S")
...
class CreateRunRequest(BaseModel):
    topic: str  # 語意是「策略方向」（公司給定，不是待發現的開放問題）
    example_config: bool = False
    n_concept_test_interviewees: int = 3
    survey_n: int = 8
...
def shared_renderers_js():
    ...
    # 同一份原始碼，不是複製過來的）——stage15-market-fit/build_replay.py 已經在載入
...
def post_company_research(body: CompanyResearchRequest):
    # 設定階段的一次性動作，不是圖節點——同步呼叫即可，不需要 run_worker.py
    # 的 subprocess/checkpoint 機制（沒有 HITL 需求，一次 10-30 秒的呼叫
    # 前端轉圈可以接受）。刻意不自動覆蓋 company.md，見 write_company_profile
    # 呼叫端（前端）自己決定要不要 cp。
    if not body.name.strip():
        raise HTTPException(400, "公司名稱不能是空的")
    result = company_research.research_company(body.name.strip(), body.urls)
    slug = body.slug.strip() or _company_slugify(body.name)
    path = company_research.write_company_profile(slug, result["markdown"])
    return {"markdown": result["markdown"], "sources": result["sources"], "path": str(path), "slug": slug}
...
def create_run(body: CreateRunRequest):
    ...
    args = [sys.executable, str(WORKER_SCRIPT), "start",
            ...
            "--n-concept-test-interviewees", str(body.n_concept_test_interviewees),
            "--survey-n", str(body.survey_n)]
...
def get_replay(run_id: str):
    ...
    # 逐步重播每一步、看結構化差異對照表（stage15-market-fit/build_replay.py）。
    run_dir = _run_dir(run_id)
    state = _current_status(run_id)
    if state.get("status") != "done":

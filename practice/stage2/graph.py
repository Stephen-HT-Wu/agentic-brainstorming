"""
階段 2：平行多 persona（子圖 fan-out）

目標：`personas.yaml` 定義 3-4 人，全部平行做功課（stage 1 的做功課子圖
原封不動重用），量測「改設定檔就能換人設」「平行比循序省時」「提案彼此
明顯不同」三件事的真實數字。

對應 PLAN.md「階段 2」驗收：
- 改設定檔就能換人設（N = len(personas.yaml) 動態決定，不寫死）
- 平行比循序省時（sum(各 persona 耗時) vs 實際 wall-clock）
- 提案彼此明顯不同（pairwise embedding 距離 + 去重後獨特點子數）

本檔是 stage 1 的完整獨立副本再擴充（不 import stage1），沿用
agentic-articles 的慣例：改一個階段不牽動其他階段。跟 stage 1 的差異
集中在「父圖」部分（`Send` fan-out 取代單一 `homework` 節點）與
`main()` 的多人彙整/度量；做功課子圖本身（collect→dedup→synthesize→
draft→refine×3）逐行相同。
...
IDEA_DEDUP_THRESHOLD = 0.75        # 跨 persona 提案去重門檻（略寬鬆，提案文字比搜尋片段長）
EMBED_DIM = 256
REFINE_ROUNDS = 3
...
def load_personas() -> List[dict]:
    ...
    跟 stage 1 的關鍵差異：stage 1 只取 personas[0]（單人做功課），
    這裡回傳整份清單——會議人數／組成完全由設定檔決定，不寫死在程式裡。
    """
...
def emit_event(
    ...
    *,
    ...
    平行 fan-out 後多個 persona 的節點會同時呼叫這裡，用 lock 序列化寫檔，
    避免多執行緒同時 append 造成同一行被切成兩半。
    """
...
def proposal_text_for_embed(proposal: dict) -> str:
    """只取 BMC 的『值』，不含鍵名——九個鍵名（客群/價值主張/…）在每份提案裡都
    逐字相同，若直接塞整包 json.dumps(bmc) 進去，這些重複鍵名會佔掉不少共同
    token，把任何兩份提案的 cosine 相似度往上拉，稀釋掉真正的內容差異訊號
    （stage 2 平行多人比較 diversity 時這個誤差會被放大，值得修正）。"""
...
def build_homework_subgraph():
    """做功課子圖：collect→dedup→synthesize→draft→refine×3（跟 stage1 逐行相同）。"""
...
# ---------------------------------------------------------------------------
# 父圖：Send fan-out —— 這是 stage 2 跟 stage 1 唯一的架構差異
# ---------------------------------------------------------------------------
#
# stage 1 的父圖只有一個 `homework` 節點、invoke 一次子圖。
# 這裡改成：一個 routing 節點依 state["personas"]（長度＝設定檔人數，不寫死）
# 產生一份 Send 清單，LangGraph 對同一個 super-step 裡的多個 Send 目標會平行
# 排程（I/O-bound 的 web_search/call_llm 會釋放 GIL，真的並發）。
# 每個 worker 各自把子圖跑完，回傳的 persona_results 用 operator.add 累加合併，
# 不需要手動處理鎖 —— 這就是「動態路由 + reducer 合併」在 map-reduce 場景的用法。


class PersonaTask(TypedDict):
    """單一 Send 目標收到的輸入 —— 刻意跟父圖的 MeetingState 分開宣告，
    因為 worker 節點只需要這三個欄位，不該看到整份會議 state。"""
    topic: str
    company: str
    persona: dict
...
def fan_out_personas(state: MeetingState) -> List[Send]:
    """routing 函式：對每個 persona 各發一個 Send，目標是同一個 worker 節點。"""
    return [
...
def homework_worker(task: PersonaTask) -> dict:
    """worker 節點：對一位 persona 完整跑做功課子圖，量測這一份的實際耗時。"""
...
# ---------------------------------------------------------------------------
# Baseline：同主題一次直接問 LLM（無搜尋、無修正、無多人）
# ---------------------------------------------------------------------------
...
# ---------------------------------------------------------------------------
# 多樣性 / 去重度量（跨 persona 提案）
# ---------------------------------------------------------------------------

def pairwise_diversity(proposals: List[dict]) -> dict:
    """N 份提案兩兩 embedding 距離：回傳平均值與明細，量化『提案彼此明顯不同』。"""
    texts = [proposal_text_for_embed(p) for p in proposals]
    pairs = []
    for (i, a), (j, b) in itertools.combinations(enumerate(texts), 2):
        pairs.append({
            "a": proposals[i].get("title", f"#{i}"),
            "b": proposals[j].get("title", f"#{j}"),
            "distance": round(embedding_distance(a, b), 4),
        })
...
def dedup_proposals(proposals: List[dict], threshold: float = IDEA_DEDUP_THRESHOLD) -> List[dict]:
    """跨 persona 提案去重（跟 dedup_by_embedding 同邏輯，門檻換成提案用的）。"""
    kept: List[dict] = []
    kept_vecs: List[List[float]] = []
    for p in proposals:
...
def _role_cost(role: str) -> float:
    """單一 persona 的實際花費：usage_log 每筆都標了 call_llm 當下的 _event_role，
    平行執行時多個 persona 的呼叫會交錯進同一個 list，靠 role 精準拆分，
    不能用『總花費 / 人數』這種平均值近似（各 persona 的搜尋輪數／修正難度不同）。"""
...
def save_outputs(
    *,
    ...
    path = OUTPUT_DIR / f"stage2-run-{stamp}.json"
    path.write_text(json.dumps(out, ensure_ascii=False, indent=2), encoding="utf-8")
    (OUTPUT_DIR / "stage2-latest.json").write_text(
...
def main() -> None:
    ...
    print(f"主題：{topic}")
    print(f"Persona 人數（來自設定檔）：{len(personas)}")
    for p in personas:
        print(f"  - {p.get('name')}（{p.get('role')}）")
    print(f"事件流：{EVENTS_PATH}")
    print()

    print(f"=== A) Agent 路徑：{len(personas)} 位 persona 平行做功課（Send fan-out）===")
    ...
    bmc_all_ok = True
    for r in persona_results:
        missing = assert_bmc_complete(r["proposal"])
        status = "OK" if not missing else f"缺 {missing}"
        cost = _role_cost(_persona_label(r["persona"]))
        print(
            f"  {r['persona']['name']:<6} BMC={status}  self_score={r['proposal'].get('self_score')}  "
            f"耗時={r['elapsed_s']}s  cost=${cost:.4f}"
        )
        bmc_all_ok = bmc_all_ok and not missing

    print()
    print(f"平行 wall-clock：{timing['parallel_wall_s']}s")
    print(f"循序估計（各 persona 耗時加總）：{timing['sequential_estimate_s']}s")
    print(f"加速倍率：{timing['speedup_x']}x")
    print()
    ...
    # 硬驗收：結構性條件用 assert，數字判斷（多樣性夠不夠）用印出來給人看，
    # 跟 stage1 的 judge_third_round() 是同一種「敘事判斷不做成硬 gate」精神。
    ok = (
        len(personas) >= 3
        and bmc_all_ok
        and len(persona_results) == len(personas)
        and timing["speedup_x"] > 1.0  # 平行真的比循序快，不是只有理論上快
        and out_path.exists()

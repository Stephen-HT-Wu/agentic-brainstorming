"""
階段 4：發表與互評（多輪共享狀態、結構化互評）

目標：homework 階段（stage1-3 的做功課→訪談→提案）結束後，N 位 persona
輪流發表（固定順序，不是動態路由——那是 stage 6 的事），其他人各給恰好
3 個認同／3 個異議／3 個洞見，並檢查提案有沒有回應自己的 HMW；發表者
針對『異議』做實質修正，形成可歸因（回應了誰的異議）的版本歷史。

對應 PLAN.md「階段 4」驗收：
- 每則互評恰好 3/3/3
- 點子池有版本歷史（被誰的異議改良，可驗證不是隨口編的歸因）

本檔是 stage 3 的完整獨立副本再擴充（不 import stage3）。做功課子圖
（collect→…→refine×3）逐行相同；新增的部分集中在父圖裡，homework 階段
的 Send fan-out 結束後多接一個 `run_presentation_rounds` 節點——這是
本階段第一次出現「多個 agent 真的看得到彼此的產出並互相回應」，跟
stage2/3 的『平行但互不知情』fan-out 是質的不同。
...
def extract_json_object(text: str) -> dict:
    """extract_json 底層是 json.loads，合法 JSON 不保證是 object——stage3 真實
    跑測踩過一次 list 導致整場崩潰，這裡統一擋掉，回傳非 dict 一律視為解析失敗。"""
    try:
...
# ---------------------------------------------------------------------------
# 發表與互評子圖（新，stage 4 核心）
# ---------------------------------------------------------------------------
#
# 每一輪只服務一位發表者：N-1 位審閱者平行（Send）給結構化意見，
# 全部到齊後發表者才修正——跟做功課階段的 fan-out 用同一個機制，
# 差別在於這次 N-1 個 Send 目標『看的是同一份提案』，回傳的意見會匯聚
# 回發表者的修正輸入，這是本階段唯一的新架構。
...
def _pad_to_three(items: Optional[list], filler_prefix: str) -> List[str]:
    """結構性保底：互評的三個欄位一律恰好 3 項，模型給少了就補齊、給多了就截斷。"""
...
def _ensure_review_shape(data: dict, reviewer: dict, presenter_name: str) -> dict:
    return {
        ...
        "hmw_addressed_reason": (data.get("hmw_addressed_reason") or "").strip(),
    }
...
def give_feedback(task: ReviewTask) -> dict:
    """審閱者對發表者的提案給結構化意見——恰好 3 認同/3 異議/3 洞見，
    外加『這個提案有沒有回應它自己的 HMW』的檢核（stage3 新增的欄位）。"""
    ...
    system = (
        ...
        f"正在聽 {presenter_name} 發表提案，要給結構化意見。"
...
def _ensure_revision_fields(revised: dict, original: dict, reviews: List[dict]) -> dict:
    """POV/HMW/insight_refs 在這個階段不重新生成——peer review 針對的是商業提案
    本身（BMC/敘事），不是需求定義；code 直接沿用原值，避免模型順手改動。
    addressed_reviewer_ids 只保留真的在這輪收到意見的審閱者 id，這是本階段
    『版本歷史可歸因』驗收的資料來源，跟 stage3 的 insight_refs 同一種設計。

    真實跑測踩過一次：prompt 明講要填 reviewer id（如 "alex"），但模型在
    addressed_reviewer_ids 裡混填了中文姓名（如 "陳建宏"），嚴格比對 id
    集合會把這些名字全部濾掉，明明 revision_note 敘事裡三個人都提到了，
    結構化欄位卻只留下 1 個——因此同時建一份 name→id 對照表，用 id 或
    姓名都能解析成正確的 id，不強迫模型精準吐出我們內部的識別碼字串。"""
    ...
    if not (revised.get("hmw_response") or "").strip():
...
# ---------------------------------------------------------------------------
# 父圖：做功課 Send fan-out（同 stage2/3）→ 發表與互評（新，固定順序、逐一進行）
# ---------------------------------------------------------------------------
...
def run_presentation_rounds(state: MeetingState) -> dict:
    """固定順序（＝ personas.yaml 的順序）逐一發表——動態決定順序是 stage 6
    才加的東西，這裡先把『多輪共享狀態＋結構化互評＋可歸因修正』這個新概念
    單獨跑通。每位發表者的審閱由 review_round_graph 內部平行處理。"""
    personas = state["personas"]
    proposal_pool = {r["persona"]["id"]: dict(r) for r in state["persona_results"]}

    review_log: List[dict] = []
    idea_pool_versions: List[dict] = []

    for presenter in personas:
        pid = presenter.get("id")
        current_result = proposal_pool[pid]
        current_proposal = current_result["proposal"]
        reviewers = [p for p in personas if p.get("id") != pid]
        emit_event(
            "present",
            f"{presenter.get('name')} 發表《{current_proposal.get('title', '')}》",
            role=_persona_label(presenter),
        )
        print(f"  [present] {presenter.get('name')} 發表《{current_proposal.get('title', '')}》")
        ...
        dist = embedding_distance(
            proposal_text_for_embed(current_proposal), proposal_text_for_embed(revised)
        )
        review_log.extend(reviews)
        idea_pool_versions.append({
            "persona_id": pid,
            "persona_name": presenter.get("name"),
            "before_title": current_proposal.get("title"),
        ...
        proposal_pool[pid]["proposal"] = revised

    emit_event(
        "run_presentation_rounds",
        f"完成 {len(personas)} 場發表與互評，共 {len(review_log)} 則評論",
    )
    return {
        "review_log": review_log,
        "idea_pool_versions": idea_pool_versions,
        "final_proposals": {pid: r["proposal"] for pid, r in proposal_pool.items()},
    }
...
def build_parent_graph():
    ...
    g.add_node("run_presentation_rounds", instrument("run_presentation_rounds", run_presentation_rounds))
    g.add_conditional_edges(START, fan_out_personas, ["homework_worker"])
    g.add_edge("homework_worker", "run_presentation_rounds")
    g.add_edge("run_presentation_rounds", END)
    return g.compile()
...
def save_outputs(
    *,
        ...
        "persona_results": [
            {
                ...
                "pov": r["pov"],
                "hmw": r["hmw"],
                "proposal_before_review": r["proposal"],
                "proposal_after_review": final_proposals.get(r["persona"]["id"]),
    ...
    path = OUTPUT_DIR / f"stage4-run-{stamp}.json"
    path.write_text(json.dumps(out, ensure_ascii=False, indent=2), encoding="utf-8")
    (OUTPUT_DIR / "stage4-latest.json").write_text(
...
def main() -> None:
    ...
    print(f"=== A) 做功課＋訪談＋提案（Send fan-out，同 stage2/3）===")
    ...
    review_log = agent_state["review_log"]
    idea_pool_versions = agent_state["idea_pool_versions"]
    final_proposals = agent_state["final_proposals"]

    sequential_estimate = sum(r["elapsed_s"] for r in persona_results)
    homework_wall_s = wall_elapsed - node_times.get("run_presentation_rounds", 0.0)
    speedup = sequential_estimate / homework_wall_s if homework_wall_s > 0 else 0.0
    timing = {
        "homework_parallel_wall_s": round(homework_wall_s, 2),
        "homework_sequential_estimate_s": round(sequential_estimate, 2),
        "homework_speedup_x": round(speedup, 2),
        "presentation_wall_s": round(node_times.get("run_presentation_rounds", 0.0), 2),
        "total_wall_s": round(wall_elapsed, 2),
    }
    ...
    after_proposals = [final_proposals[r["persona"]["id"]] for r in persona_results]
    ...
    print()
    print("=== 驗收 ===")
    expected_review_count = len(personas) * (len(personas) - 1)
    reviews_shape_ok = all(
        len(r["agreements"]) == 3 and len(r["disagreements"]) == 3 and len(r["insights"]) == 3
        for r in review_log
    )
    print(f"互評總數：{len(review_log)}（預期 {len(personas)}×{len(personas)-1}={expected_review_count}）")
    print(f"每則互評恰好 3/3/3：{'是' if reviews_shape_ok else '否'}")
    print()

    version_history_ok = True
    for v in idea_pool_versions:
        n_addr = len(v["addressed_reviewer_ids"] or [])
        changed = v["embedding_distance"] > 0.0
        print(
            f"  {v['persona_name']:<6} 回應 {n_addr} 位審閱者的異議  "
            f"embed_dist={v['embedding_distance']}  修正：{v['revision_note']}"
        )
        version_history_ok = version_history_ok and n_addr >= 1 and changed

    print()
    print(f"提案多樣性（互評前）：{diversity_before['avg_distance']}")
    print(f"提案多樣性（互評後）：{diversity_after['avg_distance']}")
    print()
    print(f"做功課平行 wall-clock：{timing['homework_parallel_wall_s']}s（循序估計 "
          f"{timing['homework_sequential_estimate_s']}s，加速 {timing['homework_speedup_x']}x）")
    print(f"發表與互評耗時：{timing['presentation_wall_s']}s（固定順序，刻意不平行——動態路由留給 stage 6）")
    print()
    ...
    ok = (
        len(personas) >= 3
        and len(review_log) == expected_review_count
        and reviews_shape_ok
        and version_history_ok
        and timing["homework_speedup_x"] > 1.0
        and out_path.exists()

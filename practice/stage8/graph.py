"""
階段 8：收斂 + Prototype + Test（多 agent 判斷聚合、原型生成、概念驗證訪談）

目標：會議收斂、大師點評、集體智慧寫入之後，全體 persona 交叉評分彼此的
最終提案（多 agent 判斷聚合），選出 top-K；每個 top 點子產出一頁概念書
＋可直接開啟的 mock landing page HTML（Prototype）；再對模擬使用者做一輪
概念驗證訪談，依真實反應做最終修正（Test），修正前後可 diff。

對應 PLAN.md「階段 8」驗收：
- 評分聚合有分歧度數字（標準差，不是只有平均分）
- 原型可直接開啟（真的寫出 .html 檔案，不是純文字描述）
- 測試訪談的反應真的改變了最終版本（embedding 距離 > 0 且有實際文字 diff）

本檔是 stage 7 的完整獨立副本再擴充（不 import stage7）。做功課子圖、
同儕互評子圖、Facilitator、HITL、大師點評、Chroma 集體智慧逐行沿用
stage7；新增的部分集中在 `write_wisdom` 之後：`run_collective_scoring`
（新子圖 `scoring_panel_graph`）→ `run_prototype_test`（新子圖
`prototype_test_graph`）。
...
CHECKPOINT_DB_PATH = OUTPUT_DIR / "stage8_checkpoints.sqlite"
...
def build_parent_graph(checkpointer):
    ...
    g.add_edge("run_prototype_test", END)
...
def save_outputs(
    *,
    ...
    path = OUTPUT_DIR / f"stage8-run-{stamp}.json"
    path.write_text(json.dumps(out, ensure_ascii=False, indent=2), encoding="utf-8")
    (OUTPUT_DIR / f"stage8-latest-{topic[:12]}.json").write_text(
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Stage 8：收斂 + Prototype + Test")
    ...
    ok = (
        ...
        and total_cost() <= MAX_BUDGET_USD + 0.5  # prototype/test 階段的花費不算進 facilitator 的預算控管
        and out_path.exists()

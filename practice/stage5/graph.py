"""
階段 5：HITL 提問（`interrupt()` + `SqliteSaver` 跨 process 續跑）

目標：每位 persona 發表後暫停整個 graph，人類可以連續提問（persona 逐一
回答）或直接跳過；提問／回答進入共享狀態，之後的同儕互評看得到；整場
用 `SqliteSaver` 落地存檔，process 中途結束也能用同一個 thread_id 從
斷點繼續跑完，不需要重跑前面已經花錢做完的步驟。

對應 PLAN.md「階段 5」驗收：
- 每人發表後暫停：人類可連續提問（agent 回答）或跳過
- 提問後 agent 的回答進入共享狀態、被後續互評引用
- 支援中斷後重開 process 續跑
- 跳過不影響流程

## 架構關鍵：為什麼發表迴圈從『一個節點裡的 Python for 迴圈』改成『圖層級迴圈』

stage4 的 `run_presentation_rounds` 是單一節點，內部用 Python `for` 迴圈
跑過每一位發表者。`interrupt()` 的語意是：graph 暫停之後，**恢復時會把
「目前卡住的那個節點函式」從頭重新執行一次**（先前呼叫過的 `interrupt()`
會立刻回傳快取值、不會真的再暫停，但那些呼叫之間的一般程式碼——包括
真正的 LLM 呼叫——會被重新執行、重新花錢）。如果把 `interrupt()` 塞進
stage4 那種一個節點跑完全部發表者的大迴圈裡，每次恢復都會把前面所有
發表者的整輪同儕互評都重新呼叫 LLM 跑一次，這是不可接受的浪費。

正確做法（也是這裡採用的）：把『發表者迴圈』真的搬到圖層級（用
`add_conditional_edges` 讓 `route_presenter` 決定要不要回到 `ask_question`，
跟 stage1 `refine` 的迴圈是同一種模式），並且讓 `ask_question` 這個節點
**只做一件事**——呼叫 `interrupt()`。真正會花錢的 `answer_question`
（persona 回答）被拆成獨立的下一個節點，不跟 `interrupt()` 共用同一個
節點函式，所以恢復時只有輕量的 `ask_question` 會重跑，已經完成的節點
（不管是本輪還是前面幾輪）都不會被重新執行、不會重新花錢——這是本檔
跟 stage2/3/4 最大的架構差異，其餘部分（做功課子圖、同儕互評子圖）逐行
沿用 stage4。
...
CHECKPOINT_DB_PATH = OUTPUT_DIR / "stage5_checkpoints.sqlite"
...
# ---------------------------------------------------------------------------
# 同儕互評子圖（同 stage4，擴充 qa_context：審閱者看得到人類問答）
# ---------------------------------------------------------------------------
...
def give_feedback(task: ReviewTask) -> dict:
    """審閱者對發表者的提案給結構化意見。跟 stage4 唯一的差異：這裡多帶了
    `qa_context`——如果人類在發表後問過問題，審閱者會看到那段問答，滿足
    『提問後 agent 的回答進入共享狀態、被後續互評引用』的驗收。"""
...
# ---------------------------------------------------------------------------
# 父圖：做功課 Send fan-out → [圖層級迴圈] ask_question → answer_question(可選)
#       → run_peer_review → 下一位 or 結束（新，stage 5 核心）
# ---------------------------------------------------------------------------
...
def _proposal_for_index(state: MeetingState, idx: int) -> dict:
    pid = state["personas"][idx].get("id")
...
def route_presenter(state: MeetingState):
    idx = state.get("presenter_index", 0)
    return "ask_question" if idx < len(state["personas"]) else END


def ask_question(state: MeetingState) -> dict:
    """本階段唯一呼叫 interrupt() 的節點——刻意不做任何其他事（不呼叫
    LLM、不寫檔），因為恢復時這個節點會被整個重跑一次。真正花錢的
    persona 回答被拆到 `answer_question`，那裡不含 interrupt()，恢復時
    不會被重新執行。"""
    idx = state["presenter_index"]
    presenter = state["personas"][idx]
    proposal = _proposal_for_index(state, idx)
    ...
    if isinstance(answer_signal, dict) and answer_signal.get("action") == "ask":
        question = (answer_signal.get("question") or "").strip()
        if question:
...
def answer_question(state: MeetingState) -> dict:
    """persona 針對人類的提問回答——真正花錢的地方，但不含 interrupt()，
    只會在真的收到新問題時執行一次，不會因為之後的恢復被重跑。"""
    idx = state["presenter_index"]
    presenter = state["personas"][idx]
    proposal = _proposal_for_index(state, idx)
...
def run_peer_review(state: MeetingState) -> dict:
    ...
    emit_event(
        "present",
        f"{presenter.get('name')} 發表《{proposal.get('title', '')}》"
        f"（人類問了 {len(qa_context)} 題）",
        role=_persona_label(presenter),
    )
...
def build_parent_graph(checkpointer):
    ...
    g.add_conditional_edges("homework_worker", route_presenter, {"ask_question": "ask_question", END: END})
    g.add_conditional_edges(
    ...
    g.add_conditional_edges("run_peer_review", route_presenter, {"ask_question": "ask_question", END: END})
...
def save_outputs(
    *,
    ...
    path = OUTPUT_DIR / f"stage5-run-{stamp}.json"
    path.write_text(json.dumps(out, ensure_ascii=False, indent=2), encoding="utf-8")
    (OUTPUT_DIR / "stage5-latest.json").write_text(
...
# ---------------------------------------------------------------------------
# CLI 驅動：互動式 stdin、腳本化（--script）、跨 process 續跑
# ---------------------------------------------------------------------------
...
def get_human_input(payload: dict, script: Optional[list]) -> dict:
    idx = payload["presenter_index"]
    asked = payload["questions_asked_so_far"]
    if script is not None:
        entry = script[idx] if idx < len(script) else {"skip": True}
        if entry.get("skip"):
            return {"action": "skip"}
    ...
    *,
    ...
    while True:
        ...
        if stop_after_first_interrupt:
            ...
            print(f"稍後可用：--thread {config['configurable']['thread_id']} 從這裡繼續。")
            return None
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Stage 5：HITL 提問（interrupt + SqliteSaver）")
    parser.add_argument("--thread", default=None, help="thread_id；同一場會議中斷後要接續，兩次要給同一個值")
    parser.add_argument("--script", default=None, help="JSON 腳本檔路徑（不給就走互動式 stdin）")
    parser.add_argument("--stop-after-first-interrupt", action="store_true", help="跑到第一個人類介入點就結束 process（示範中斷後續跑用）")
    ...
    after_proposals = [v["proposal_after"] for v in idea_pool_versions]
    ...
    print(f"互評總數：{len(review_log)}（預期 {expected_review_count}）；每則恰好 3/3/3：{'是' if reviews_shape_ok else '否'}")
    print(f"人類提問總數：{len(human_qa_log)}")
    for qa in human_qa_log:
        print(f"  問 {qa['presenter_name']}：{qa['question']}")
        print(f"    答：{qa['answer']}")

    qa_referenced_in_review = 0
    for v in idea_pool_versions:
        n_qa = v["human_questions_asked"]
        if n_qa > 0:
            qa_referenced_in_review += 1
        print(
            f"  {v['persona_name']:<6} 人類問了 {n_qa} 題  "
            f"回應 {len(v['addressed_reviewer_ids'] or [])} 位審閱者  "
            f"embed_dist={v['embedding_distance']}"
        )

    at_least_one_asked = len(human_qa_log) > 0
    at_least_one_skipped = len(idea_pool_versions) > len({qa["presenter_id"] for qa in human_qa_log})
    ...
    ok = (
        ...
        and len(idea_pool_versions) == len(personas)
        and at_least_one_asked  # 至少有一位真的被問過（示範『提問』這條路徑有跑到）
        and at_least_one_skipped  # 至少有一位被跳過（示範『跳過』這條路徑也有跑到）
        and out_path.exists()
